...
CPActive PActive;
    CFile File;
//增加生成是数据l文件功能，为压缩文件和清理文件准备历史数据文件
//增加信号处理函数，处理2、15
//解决调用exit函数退出时局部对象没有调用析构函数问题
//把心跳信息写入共享内存
...
int main(int argc ,char *argv[])
{
      ...
      if((argc!=5) && (argc!=6) )
      {
      cout<<"Using:./crtsurfdata inifile outpathfile logfilei datafmt [datetime]"<<endl;
      cout<<"Example:/project/idc1/bin/crtsurfdata /project/idc1/ini/stcode.ini /tmp/idc/surfdata /log/idc/crtsurfdata.log xml,json,csv 20210710123016\n\n";                                                                                      
